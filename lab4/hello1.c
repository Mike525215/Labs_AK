...
int print_hello(uint quantity)
{
    struct timing_info *element, *tmp_element;
    uint counter;

    if (quantity <= 0) {
        pr_err("Error: Invalid 'quantity' (must be positive)\n");
    } else if (quantity > 0 && quantity < 5) {
        pr_info("Note: 'quantity' is less than 5\n");
    } else if (quantity >= 5 && quantity <= 10) {
        pr_warn("Warning: 'quantity' is in the range 5-10\n");
    } else { 
        pr_err("Error: 'quantity' exceeds 10\n");
        return -EINVAL;
    }
